...
def adjust_learning_rate(optimizer, epoch, args):   
    ...
    if args.lradj == 'type1':
        ...
        lr_adjust = {epoch: args.learning_rate if epoch < 5 else args.learning_rate * (0.9 ** ((epoch - 3) // 1))}
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path,all_labels, all_preds,all_dict):
           
        score = val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, path,all_labels, all_preds,all_dict)
    ...
    def save_checkpoint(self, val_loss, model, path,all_labels, all_preds,all_dict):
        def convert_int64_keys(obj):
            ...
            if isinstance(obj, dict):
                ...
                return [convert_int64_keys(item) for item in obj]
        ...
        ts = datetime.now().astimezone().strftime("%Y-%m-%d:%H")
        path_model = path + f"model_{ts}.pth"
        print("path",path_model)
        if not path_model.endswith('.pth'):
            raise ValueError("保存路径应以 .pth 结尾")


        torch.save(model.state_dict(), path_model)

        np.save(path+f'all_labels_{ts}.npy', all_labels)
        np.save(path+f'all_preds_{ts}.npy', all_preds)


        model_name = path
           
        dict_all_result = {model_name: "ModelName"}     
        dict_all_result.update(convert_int64_keys(all_dict))     


        dict_filename = f"all_dict_{ts}.json"

        with open(path + dict_filename, 'w') as json_file:
            json.dump(dict_all_result, json_file, indent=4)
